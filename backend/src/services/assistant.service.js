...
async function buildHealthSupportResponse({ message, normalizedMessage, context, history, healthQuery = null, language = 'es' }) {
  ...
  if (!candidates.length) {
    return buildResponse(
      isEnglish(language)
        ? 'I do not see dishes that clearly fit your profile for that question. You can review your food profile or ask me for more specific menu alternatives.'
        : 'No veo platos claramente compatibles con tu perfil para esa consulta. Puedes revisar tu perfil alimentario o preguntarme por alternativas mas concretas del menu.',
      [navigateAction('/menu', language), navigateAction('/perfil', language)],
      'fallback'
    );
  }
  ...
  if (!process.env.OPENAI_API_KEY) {
    return buildResponse(
      buildHealthSupportFallbackMessage(resolvedHealthQuery, candidates, context.profile, language),
  ...
  try {
    ...
    const openAiResponse = buildOpenAiResponseOrNull(content, context, language);
